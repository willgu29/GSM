			...
			if (group.level == this.props.queryLevel) {
				...
				if (group.childrenGroup_ids) {
					for (var k = 0; k < group.childrenGroup_ids.length; k++) {
						...
						var childenGroupURL = this.props.url + "/" + group.childrenGroup_ids[k] + "?groupName=" + group.childrenGroup_names[k];
						childrenGroupItems.push(React.createElement(
							"li",
							null,
							React.createElement(
								"a",
								{ href: childenGroupURL },
								childGroupNames[k]
							)
						));
...
var GSMGroupView = React.createClass({
	displayName: "GSMGroupView",

	getInitialState: function getInitialState() {
		return { groups: [] };
	},
	loadInitialGroups: function loadInitialGroups() {
